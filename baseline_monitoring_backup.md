...
> 备份日期：2026-03-10 | 最后更新：2026-03-26（FIX-85 异常率惩罚前移到 25% 即明显生效；FIX-84 链路安全与 W 口径统一；FIX-83 A维度公平性补丁（只改A不碰W）；FIX-82 历史最高分全量回填与详情核对；FIX-81 W 日均基准个性化升级迭代）
...
### FIX-85（异常率惩罚前移到 25%）
- 背景：25% 异常率下旧惩罚曲线不够明显，仍可能高位排名
- 目标：让 25% 即出现可感知扣分，并在 25%~60% 区间加速下压
- 实现：
  - 新增 `fix85_outlier_penalty_25pct.sql`
  - 动态补丁同步更新：
    - `compute_student_score(text)`（实时）
    - `compute_student_score_as_of(text,date)`（历史）
- 新曲线：
  - `<=25%`：线性惩罚（25% 时约 `0.85`）
  - `25%~60%`：线性加速惩罚（60% 时约 `0.50`）
  - `>60%`：指数衰减继续下压
- 说明：该改动会直接影响综合分排序，建议执行后全量重算并复核前后排名变化
