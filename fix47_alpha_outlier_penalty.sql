-- ============================================================
-- FIX-47：compute_baseline_as_of + compute_baseline
-- 修复 alpha 异常率惩罚系数过小（旧 0.02×rate 最多仅扣 2%）
-- 导致高异常率学生因 cleaned_duration 截断 CV 极低，α 反而虚高
--
-- 新惩罚公式（分段线性）：
--   outlier_rate ≤ 30%: 线性   0.08 × rate     (最多 -0.024)
--   outlier_rate  > 30%: 急剧   0.024 + 0.40 × (rate - 0.30)
--
-- 典型对照（马逸诚 outlier_rate=73.3%，mean=120，CV≈0）：
--   旧 alpha ≈ 0.942   新 alpha ≈ 0.761
-- ============================================================

-- ──────────────────────────────────────────────────────────────
-- 1. compute_baseline_as_of（完整实现，含 FIX-47）
-- ──────────────────────────────────────────────────────────────
...
BEGIN
    ...
    -- FIX-47：异常率惩罚由旧的线性 0.02×rate（最大仅扣2%）改为分段加速：
    --   rate ≤ 30%：线性  0.08 × rate           （最大 -0.024）
    --   rate  > 30%：急剧  0.024 + 0.40 × (rate - 0.30)   （73% ≈ -0.20, 100% ≈ -0.30）
    -- 解决"高异常率学生因 cleaned_duration 全截断为120导致 CV≈0 → alpha虚高"的结构性问题
    v_alpha := 1.0
        -- 低均值风险惩罚
        - CASE
...
-- ──────────────────────────────────────────────────────────────
-- 2. compute_baseline（薄封装，传入明天 = 包含今天所有数据）
-- ──────────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────────
-- 步骤1：部署以上两个函数
-- 步骤2：全量重算基线（修正所有学生的 alpha）
--   SELECT public.recompute_all_baselines();
--
-- 步骤3（可选）：重算历史成长分快照（alpha 变化会通过 outlier_penalty 间接影响分数，
--               但 alpha 本身不直接参与 compute_student_score，可按需执行）
