...
| 进步榜 | 本周综合分 - 最近2个有效周最高分 | 本周工作日练琴 >= 2 次 + 绝对涨分 >= 1.0 + 近10条工作日记录 >= 4 + 近10条工作日异常率 <= 45% + 综合榜 Top10 自动退出 |
| 稳定榜 | `alpha` 降序 | `alpha >= 0.60` + 近10条工作日记录 >= 8 + 近10条工作日均时 >= 30 + 异常率 <= 35% + 综合榜 Top10 自动退出 |
| 守则榜 | `recent10_outlier_rate` 升序 | 本周工作日 >= 3 次 + 近10条工作日记录 >= 5 + 均时 >= 30 + 异常率 <= 40% + `alpha >= 0.60` + 综合榜 Top10 自动退出 |
...
| **进步榜** | `display_score - lw_composite`（**绝对涨分**）↓ | `display_score` ↓ → `mean_duration` ↓ | 有历史周快照 + 绝对涨分 ≥ 1.0 + 本周**工作日**练琴 ≥ 2次 + 近10条工作日记录 ≥ 4条 + 近10条工作日异常率 ≤ 45% + **综合榜 Top 10 自动退出** |
| **稳定榜** | `alpha`（一致性）↓ | `recent10_mean_dur` ↓ → `outlier_rate` ↑ | α ≥ 0.60 + 近10条**工作日**记录 ≥ 8条 + 近10条工作日均时 ≥ 30min + 近10条工作日异常率 ≤ 35% + **综合榜 Top 10 自动退出** |
| **守则榜** | `recent10_outlier_rate` ↑（越低越好） | `week_sessions`（本周工作日次数）↓ → `mean_dur` ↓ | 本周**工作日** ≥ 3次 + 近10条工作日记录 ≥ 5条 + 均时 ≥ 30min + 异常率 ≤ 40% + α ≥ 0.60 + **综合榜 Top 10 自动退出** |
