...
| 稳定榜 | 近20条均练时长优先 | `alpha` 次级排序 | `alpha >= 0.60` + 近20条工作日记录 >= 8 + 近20条工作日均时 >= 30 + 近20条工作日异常率 <= 20% + 综合榜 Top10 自动退出 |
| 守则榜 | `recent10_outlier_rate` 升序 | 本周工作日 >= 3 次 + 近20条工作日记录 >= 5 + 均时 >= 30 + 近20条异常率 <= 20% + `alpha >= 0.60` + 综合榜 Top10 自动退出 |
...
| **进步榜** | `display_score - lw_composite`（**绝对涨分**）↓ | `display_score` ↓ → `mean_duration` ↓ | 有历史周快照 + 绝对涨分 ≥ 1.0 + 本周**工作日**练琴 ≥ 2次 + 近10条工作日异常率 ≤ 30% + **综合榜 Top 10 自动退出** |
| **稳定榜** | `recent10_mean_dur`（近20条均练时长）↓ | `alpha` ↓ → `outlier_rate` ↑ | α ≥ 0.60 + 近20条**工作日**记录 ≥ 8条 + 近20条工作日均时 ≥ 30min + 近10条工作日异常率 ≤ 30% + **综合榜 Top 10 自动退出** |
| **守则榜** | `recent10_outlier_rate` ↑（越低越好） | `week_sessions`（本周工作日次数）↓ → `mean_dur` ↓ | 本周**工作日** ≥ 3次 + 近20条工作日记录 ≥ 5条 + 均时 ≥ 30min + 近10条异常率 ≤ 20% + α ≥ 0.60 + **综合榜 Top 10 自动退出** |
