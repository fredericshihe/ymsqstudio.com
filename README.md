...
| 范围 | 文件 / 迁移 | 说明 |
|------|-------------|------|
| 五类榜单 · 缩水榜 | `practiceanalyse.html`、`menuhin-school-system/index.html`、`排行榜指南.html` | 新增缩水榜说明：仅统计本周已开始有效练琴且当前应完成进度低于 70% 的学生；未开始学生单独统计，不参与排名。 |
| 音符币 · 自动扣币 | `20260828190000_weekly_shrink_coin_penalties.sql` | 缩水榜前 6 名按进步榜同名次额度 `-10/-6/-4` 扣币，余额允许为负；奖励与扣币共同受管理员自动结算开关控制。 |
| 在读学生口径 | `leaderboard_rpc.sql`、两个排行榜前端 | `student_database.archived=false` 是所有榜单、未练统计和前端学生展示的统一名单基准。 |
...
1. **练琴会话采集链**  
   `practice_logs -> practice_sessions -> student_baseline`

2. **评分与排行榜链**  
   `compute_student_score / compute_student_score_as_of -> student_score_history -> get_weekly_leaderboards()`
...
- 用**工作日练琴数据**计算学生画像与成长分
- 用**绝对分**而不是百分位分展示综合榜
- 用**统一北京时间边界**避免周切换误差
- 用**自动结算 + 可审计流水**管理音符币
- 用**可回测 SQL**对评分参数和 W 刷新链路做持续验收
...
6. `sql/fixes/active/fix80_trigger_chain_stable.sql`
   ...
   - 稳定策略（本地补丁目标）：`trigger_update_student_baseline` 回归纯 baseline 更新；`trigger_compute_student_score`
     设计为保留 `skip_score_trigger`、`app.computing_score`、`pg_trigger_depth()>2` 三层保护（兼容 `on/true/1`）
   - 线上核查结果（2026-05-11）：当前生产库实际已落地的是 `skip_score_trigger` + `app.computing_score`
     两层保护，尚未看到 `pg_trigger_depth()>2` 这一层
...
practice_logs
  ...
  -> practiceanalyse.html / menuhin-school-system/index.html
```
...
| 表名 | 作用 |
|---|---|
| `practice_sessions` | 每次练琴会话明细（时长、异常、房间） |
| `student_baseline` | 学生最新画像（均时长、异常率、alpha、五维分、W 等） |
| `student_score_history` | 每周历史快照 |
| `weekly_leaderboard_history` | 每周五榜单备份 |
| `student_coins` | 学生音符币总余额 + 本学期累计获得 |
| `coin_transactions` | 音符币流水 |
| `weekly_coin_reward_log` | 自动结算防重日志 |
| `system_settings` | 自动发币开关等系统配置 |
...
- `raw_score`：0~1 的原始成长值
- `composite_score = ROUND(raw_score * 100, 1)`
- 已取消 `PERCENT_RANK()` 百分位归一化
- 所有周切换、回填、榜单口径统一按 `Asia/Shanghai`
- 周六、周日不计入当前周排行榜资格，也不计入近10条统计与基线

### 4.2 五维说明

| 维度 | 含义 | 当前逻辑摘要 |
|---|---|---|
| `B` | 进步分 | 最近 1 个活跃周 vs 前 1 个活跃周，变化分量 80%，绝对水平分量 20% |
| `T` | 趋势分 | 最近 2 个活跃周均值 vs 前 2 个活跃周均值，变化分量 80%，绝对水平分量 20% |
| `M` | 动量分 | 最近 4 个活跃周是否达到“个人周均目标”的加权达标率 |
| `A` | 底盘分 | 样本量 × 同专业质量的长期练琴底盘 |
| `W` | 本周进度分 | 本周工作日累计进度相对个人日均目标的实时完成度 |
...
W 是“**到当前这一天为止，这周进度有没有跟上**”。

计算步骤：

1. 统计本周工作日累计分钟  
   `weekly_mins = 本周一到现在 cleaned_duration 累计（只算周一到周五）`

2. 计算 W 日均基准  
   不是直接用 `mean_duration`，而是：

```text
alpha = LEAST(1, record_count / 15)
effective_mean = alpha * mean_duration + (1 - alpha) * median_mean
w_daily_ref = GREATEST(effective_mean, 30)
```

其中：

- `median_mean`：优先取**同专业中位数**；同专业样本不足 5 人时退化为全体中位数
- `effective_mean`：贝叶斯收缩后的个人日均
- `w_daily_ref`：W 真正使用的分母日均，且最低不低于 30 分钟/天
...
```text
ratio = weekly_mins / (w_daily_ref * elapsed_days)
...
1. **防新生低基数虚高**  
   记录少时，不会因为偶然练得短而让分母过低，导致 W 很容易爆高

2. **防极低均值刷分**  
   就算个人历史均值很低，也不会让 W 分母低于 `30`

3. **让系统从“群体先验”平滑过渡到“个人真实习惯”**

一个具体例子：

- 同专业中位数 `80`
- 新生个人均值 `20`
- 只有 `2` 条记录，`alpha = 2/15 = 0.133`

则：

```text
effective_mean = 0.133*20 + 0.867*80 ≈ 72
w_daily_ref = max(72, 30) = 72
```
...
- **异常率惩罚**：`outlier_rate <= 60%` 线性惩罚，超过后指数衰减
- **peak_decay**：防止吃历史高峰老本，但不会把总分砍到失真
...
| 榜单 | 排序字段 | 上榜条件 |
|---|---|---|
| 综合榜 | `display_score` 降序 | 本周有工作日练琴 + 综合分 > 0 |
| 进步榜 | 本周综合分 - 最近2个有效周最高分 | 本周工作日练琴 >= 2 次 + 绝对涨分 >= 1.0 + 近20条工作日异常率 <= 20% + 综合榜 Top10 自动退出 |
| 稳定榜 | `alpha` 降序 → 时长变异系数升序 → 异常率升序 | `alpha >= 0.60` + 最近20条工作日记录 >= 8 + 均时 >= 30 + 异常率 <= 20% + 综合榜 Top10 自动退出 |
| 守则榜 | 异常率升序 → 本周不同工作日数降序 | 本周至少 3 个不同工作日 + 最近20条工作日记录 >= 5 + 均时 >= 30 + 异常率 <= 20% + `alpha >= 0.60` + 综合榜 Top10 自动退出 |
| 缩水榜 | 当前应完成进度升序 | 本周已有有效练琴 + 本周累计有效分钟 / 当前工作日理论应完成分钟 < 70%；未开始学生不参与 |

### 5.2 当前关键原则

- 进步榜看**绝对涨分**，不看百分比涨幅
- 进步榜基准取最近 2 个有效周最高分，避免节后低基数虚高
- 综合榜 Top 10 自动退出专项榜，避免长期垄断多个榜单
- 缩水榜完成率越低排名越靠前，前 6 名按 `-10/-6/-4` 扣币；余额不足时允许显示负数
- 所有榜单和未练统计只使用 `student_database.archived=false` 的在读学生

### 5.3 前端展示文件

| 文件 | 作用 |
|---|---|
| `practiceanalyse.html` | 管理员大盘 + 学生详情 + 五维分解 |
| `menuhin-school-system/index.html` | 学生端排行榜、Tips、第二页规则摘要 |
| `排行榜指南.html` | 可打印完整版排行榜规则 |
| `leaderboard_rpc.sql` | 四个正向榜单后端逻辑源码；缩水榜由独立 RPC 提供 |
...
| 函数 / 视图 | 作用 |
|---|---|
| `adjust_student_coins()` | 原子更新余额并写流水 |
| `vw_student_coin_balances` | 学生信息 + 余额 + 学期累计 |
| `reward_weekly_coins()` | 每周五自动发放正向榜奖励并扣除缩水榜前 6 名音符币 |
| `set_auto_reward_enabled()` / `get_auto_reward_setting()` | 奖励与缩水榜扣币的统一自动结算开关 |
...
| 任务 | 北京时间 | 作用 |
|---|---|---|
| `backup_weekly_leaderboards_job` | 周五 21:30 | 备份本周榜单 |
| `reward_weekly_coins_job` | 周五 21:32 | 自动结算发币 |
| `weekly_score_update_job` | 周五 21:35 | 写入最终周快照 |

### 7.2 W 分兜底任务

| 任务 | 北京时间 | 作用 |
|---|---|---|
| `refresh_w_score_monday_bootstrap` | 周一 00:10 | 开周补刷一次 |
| `refresh_w_score_weekday_daily` | 工作日 20:05 | 每天一次兜底刷新 |
...
- `supabase/functions/batch-ai-analysis/index.ts`
- 周一到周五批量运行
- 周六、周日默认不自动触发（除非 `force_run = true`）
...
| 文件 | 作用 |
|---|---|
| `practiceanalyse.html` | 管理员大盘与学生详情 |
| `admin_coins.html` | 音符币后台 |
| `兑换券.html` | 兑换券展示/打印 |
| `menuhin-school-system/index.html` | 学生端首页 |

维护备注：

- 自 `2026-04-28` 起，学生端前端页面已同步纳入当前仓库路径 `menuhin-school-system/index.html`，后续可在本项目内集中维护。
- 外部独立仓库 `menuhin-school-system` 仍保留，用于学生端页面单独推送与发布；如在本仓库修改学生端页面，请同步回推对应远端仓库。
...
- `practiceanalyse.html`：本地优先 + CDN 兜底
- `admin_coins.html`：本地 `supabase.min.js` 优先 + `jsdelivr/unpkg/skypack` 兜底

这样即使 CDN 超时，页面也不会一直停在“连接服务器”。

### 8.3 学生详情页 W 展示

`practiceanalyse.html` 中，W 卡片现已显示：
...
并且移除了容易误导的“个人原始日均（mean_duration）”直出文案。
...
- `backtest_score_reasonableness.sql`
  - 查 `raw/composite` 一致性
  - 查范围越界
  - 查历史异常明细
  - 查综合榜“长期霸榜”风险
...
- `backtest_dimension_thresholds.sql`
  - B/T/M/A/W 分布
  - 饱和率 / 区分度
  - M 阈值敏感性
  - A 单调性
...
- `backtest_leaderboard_min_thresholds.sql`
  - 三榜（进步/稳定/守则）历史上榜样本分布
  - 最低门槛参数组对比（`baseline_old` / `fix_lb_80` / `strict_high`）
  - 空榜率与通过率评估（防止参数过松或过严）

### 9.3 W 专项核对

- `verify_w_dimension_alignment.sql`
  - 核对线上函数定义
  - 查 `w_score_updated_at`
  - 查 W 与 `progress_ratio` 的一致性
  - 查异常样本
...
## 10. 当前验收结论（2026-03-23）

### 已确认通过

- `raw_score` 与 `composite_score` 一致
- W 公式与线上存储一致
- `w_score_updated_at` 已建立
- W 兜底刷新任务已生效
- `practiceanalyse.html` 的 W 展示口径已与后端对齐
- `admin_coins.html` 已改为本地库优先，弱网下更稳

### W 维度当前状态

- 公式一致性通过
- `suspicious_cnt = 0`
- `w_updated_since_week_start = 192/192`
- 相关性通过：
  - `corr_w_vs_ratio = 0.9529`
  - `corr_w_vs_sigmoid_ratio = 0.9999`

结论：  
**W 已达到“计算正确 + 时效完整 + 可观测 + 可回测”的状态。**
...
- **综合分**：看这周整体表现，满分 100
- **W 分**：看“到今天为止，这周进度有没有跟上”
- **音符币**：每周五按榜单自动发，也可以管理员手动加减
- **README.md**：以后只看这一份说明就够了

---

## 14. 附录A：`系统架构文档.md` 全文镜像

> 说明：以下内容为 `系统架构文档.md` 的完整并入版，保留原始结构，便于在单一 `README.md` 中统一查阅。

# 琴房管理系统 — 排行榜 & 音符币系统架构文档

> 文档治理说明（2026-03-23）：`README.md` 现已整理为当前**统一说明文档 / 单一阅读入口**。  
> 本文件保留为扩展架构说明与历史整理参考，不再作为唯一维护入口。

> 最后更新：2026-03-23（README 统一整理版已建立；W 可观测性、兜底刷新、前端依赖容灾已同步）
...
│  start_new_semester          coin_transactions                  │
│  adjust_student_coins        weekly_leaderboard_history         │
...
由实时触发器持续覆盖本周快照、并由周任务补齐未练学生的**周快照表**，记录每位学生每周的各项维度分数。
...
| `alpha` | NUMERIC | 可信度 α |
| `trend_score` | NUMERIC | **进步榜**：本周综合分 − 近2个有效周最高分（绝对涨分）；**其余三榜**：真实趋势分（0~1）|
...
```
practice_sessions ──► recent10（近12周最多10条工作日记录）
                           │
student_baseline ──────────┤
student_score_history ─────┤
                           ▼
practice_sessions ──► week_cnt（本周工作日次数）
                           │
...
| **进步榜** | `display_score - lw_composite`（**绝对涨分**）↓ | `display_score` ↓ → `mean_duration` ↓ | 有历史周快照 + 绝对涨分 ≥ 1.0 + 本周**工作日**练琴 ≥ 2次 + 最近20条工作日记录 ≥ 4条 + 异常率 ≤ 20% + **综合榜 Top 10 自动退出** |
| **稳定榜** | `alpha` 降序 → 时长变异系数（CV）升序 | `outlier_rate` 升序 → `recent10_mean_dur` 降序 | α ≥ 0.60 + 最近20条**工作日**记录 ≥ 8条 + 最近20条均时 ≥ 30min + 最近20条异常率 ≤ 20% + **综合榜 Top 10 自动退出** |
| **守则榜** | `recent10_outlier_rate` 升序（越低越好） | 本周不同工作日数降序 → `mean_dur` 降序 | 本周至少 **3 个不同工作日**有练琴 + 最近20条工作日记录 ≥ 5条 + 均时 ≥ 30min + 最近20条异常率 ≤ 20% + α ≥ 0.60 + **综合榜 Top 10 自动退出** |

**关键概念：**
- **α（基线可信度）**：基于最近 30 条**工作日**记录的均值、波动、异常率、短时率综合计算；冷启动阶段再与同专业/同年级群体均值做混合，最终截断在 `0.5 ~ 1.0`
...
- **进步榜排序键**：按**绝对涨分**（`display_score - lw_composite`）降序，而非百分比涨幅。防止上周几乎不练的学生以极低基数制造虚高百分比霸榜。`trend_score` 列当前直接存储**绝对涨分**，前端展示为 `+X.X 分`。
...
页面加载 init()
  └── 并行执行：
        ...
        └── loadSemesterData()
              └── vw_student_coin_balances → 按 semester_earned 倒序前 20 名并渲染学期累计排行
...
> 线上核查注记（2026-05-11）：
> 当前生产库中，`get_auto_reward_setting()` 与 `set_auto_reward_enabled()` 仍对 `anon/authenticated`
> 保持可执行，以支持 `admin_coins.html` 直接读写开关；`reward_weekly_coins()` 仍仅限 `service_role` 执行。
...
| 名次 | 综合榜 | 稳定榜 | 守则榜 | 进步榜 |
|------|--------|--------|--------|--------|
| 第 1 名 | 80 币（榜首霸主） | 12 币 | 11 币 | 10 币 |
| 第 2–3 名 | 64 币（荣耀亚军） | 9 币 | 8 币 | 6 币 |
| 第 4–6 名 | 48 币（实力季军） | 5 币 | 5 币 | 4 币 |
| 第 7–10 名 | 32 币（优秀达人） | — | — | — |
...
| `admin_coins.html` | 前端 | 音符币管理后台；手动加减币 + 流水查看 + 自动发放开关 + **学期累计排行/新学期重置** |
| `兑换券.html` | 前端 | 可打印的音符币兑换券 |
| `排行榜指南.html` | 前端 | 可打印的排行榜规则 + 奖励说明 |
...
| `verify_semester_simplification.sql` | SQL | 学期管理精简一键验证：检查对象下线、函数可用、视图字段与累加逻辑是否生效 |
...
| 2026-03-19 | **音符币自动结算**：新增 `weekly_coin_reward_log` 防重复表、`reward_weekly_coins()` 结算函数（遍历四榜按名次发币，流水 `reason` 含榜单/名次/称号/分数等详细说明，`p_type='auto_reward'`）、pg_cron 任务 `reward_weekly_coins_job`（UTC `32 13 * * 5`，BJT 每周五 21:32；后续已提前为 2026-03-27 生效，文档已按线上状态修正） | `setup_coin_rewards.sql`、`系统架构文档.md` |
| 2026-03-19 | **三榜排序公平性优化（历史记录，后续已被 FIX-58 覆盖）**：① 进步榜曾改为相对涨幅排序 `(本周-上周)/上周×100%`，`trend_score` 存储百分比（保留1位小数），前端显示 `+XX.X%`；② 稳定榜主排序从 `alpha`（资历偏向）改为 `recent10_mean_dur DESC`（近10次均时），α 降为次级排序；③ 守则榜并列决胜从 `record_count`（历史总次数，资历偏向）改为 `week_sessions DESC`（本周练琴次数）。 | `leaderboard_rpc.sql`、`practiceanalyse.html`、`menuhin-school-system/index.html`、`系统架构文档.md` |
| 2026-03-19 | **进步榜排序键改绝对涨分（FIX-58）**：排序从百分比涨幅改为绝对涨分 `display_score - lw_composite`，防止低基数学生（上周几乎不练）凭虚高百分比霸榜。同步收紧上榜门槛：上周综合分 ≥ 35、本周 ≥ 45、本周练琴 ≥ 3次、涨幅 ≥ 5分、异常率 ≤ 40%。`trend_score` 在进步榜语义下同步存储绝对涨分，前端显示 `+X.X 分`。 | `leaderboard_rpc.sql`、`系统架构文档.md` |
...
| 2026-03-26 | **FIX-80 触发链稳定版（本地推荐补丁）**：基于实际报错 `stack depth limit exceeded`，本地补丁设计为：① `trigger_update_student_baseline` 回归纯 baseline 更新；② `trigger_compute_student_score` 增加三层防护（`skip_score_trigger` / `app.computing_score` / `pg_trigger_depth()>2`）并兼容 `on/true/1`。按 2026-05-11 线上核查，生产库当前已确认前两层存在，但未看到 `pg_trigger_depth()>2` 这一层。 | `fix80_trigger_chain_stable.sql`、`README.md`、`baseline_monitoring_backup.md` |
...
| α 门槛 | `>= 0.65` | `>= 0.55`（略微放宽） |
| 近20条数量 | `>= 10` | `>= 8`（近12周约每10天一次即可） |
| 异常率 | `<= 0.30` | `<= 0.30`（收紧到新口径） |
...
同步更新副标题：
- 进步榜：`"本周综合分相对涨幅最大 · α ≥0.50"` → `"本周综合分绝对涨分最大 · 本周 ≥2 次 · 近20条异常率 ≤30%"`
- 稳定榜：近20条均时优先，α ≥0.60，近20条 ≥8，异常率 ≤30%
- 守则榜：α ≥0.60，近20条 ≥5，均时 ≥30min，异常率 ≤20%
