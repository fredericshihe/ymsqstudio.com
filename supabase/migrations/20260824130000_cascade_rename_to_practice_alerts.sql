-- sync_student_name_references() (see 20260616133000, patched in 20260824120000) never
-- included public.practice_alerts in its rename cascade. A student rename therefore left
-- any of their existing alert rows pointing at the old name — same failure mode as the
-- historical student_time_slots corruption that produced "专业 姓名"-style alert tags,
-- just triggered by a normal rename instead of a bad import. Add it to the cascade.
